...
import Nfttokenabi from "../../../abis/NFTokenABI.json";
import OpenMarketplaceABI from "src/abis/OpenMarketplaceABI.json";
...
import { PlaneListPopup } from "../Profile/Profile";
...
export default function Login({ data, index, callbackFn }) {
          ...
          if (parseFloat(auctionDetails?.startingBid) <= parseFloat(bid)) {
            ...
            try {
              ...
              if (parseFloat(auth?.userData?.massBalance) > 0) {
                if (parseFloat(auth?.userData?.massBalance) > parseFloat(bid)) {
                  ...
                  toast.error(
                    "Your MASS balance is insufficient for placing a bid "
                  );
                  setIsBidding(false);
                }
              } else {
                toast.error("Your MASS balance is too low.");
                setIsBidding(false);
              }
              // } else {
  ...
  const buyNowhandler = async () => {
    try {
      ...
      // if (auth?.userData?.userType === 'Creator') {
      if (
        parseFloat(auth?.userData?.massBalance) >=
  ...
  return (
    <Box className={classes.LoginBox}>
            ...
            <>
              <Container maxWidth="lg">
                ...
                <Box mb={3} className="seats">
                  <label className={classes.labelText}>{t("bundles.details")}</label>
                ...
                <Grid container style={{ alignItems: "center" }}>
                  <Grid item xs={12} md={1}>
                    <label className={classes.labelText}>{t("bundles.timeLeft")}</label>
                  </Grid>
                  <Grid item xs={12} md={11}>
                    {auctionDetails?.isExport ? (
                      <List className={classes.timeing}>
                        <ListItem>{t("bundles.exported")}</ListItem>
                      </List>
                    ) : (
                      <>
                        ...
                        <>
                          {auctionDetails?.nftId?.userId !== ProfileId &&
                            ...
                            !auctionDetails?.isCancel && (
                              ...
                              >
                                {t("bundles.buyNow")} {isBuyBid && <ButtonCircularProgress />}
                              </Button>
                            )}
                          &nbsp;&nbsp;
                          {auctionDetails?.nftId?.userId !== ProfileId &&
            ...
            <DialogContentText id="alert-dialog-slide-description">
              ...
              <Box className={classes.timeBox}>
                ...
                <Box item xs={12} md={11}>
                  <List className={classes.timeing}>
                    <ListItem>
                      {closeTimeLeft?.days ? closeTimeLeft?.days : "0"} {t("bundles.d")}
                    </ListItem>
                    <ListItem>
                      {closeTimeLeft?.hours ? closeTimeLeft?.hours : "0"} {t("bundles.h")}
                    </ListItem>
                    <ListItem>
                      {closeTimeLeft?.minutes ? closeTimeLeft?.minutes : "0"} {t("bundles.m")}
                    </ListItem>
                    <ListItem>
                      {closeTimeLeft?.seconds ? closeTimeLeft?.seconds : "0"} {t("bundles.s")}
                    </ListItem>
                  </List>
                </Box>
              </Box>
        ...
        >
          <DialogContent className={classes.paper}>
            <DialogContentText id="alert-dialog-slide-description">
              ...
              <Box>
                <label>{t("bundles.selectAProblem")}</label>
                <Box className="reposr-btn">
                  <Button>NSFW</Button>
                  <Button>{t("bundles.antiReligions")}</Button>
                  <Button className="active">{t("bundles.copyrightViolation")}</Button>
                  <Button>{t("bundles.other")}</Button>
                </Box>
              </Box>
              <Box mt={4}>
                <label>{t("bundles.comment")}</label>
                <TextField
            ...
            <DialogContentText id="alert-dialog-slide-description">
              ...
              <Box mt={2} className={classes.btnBox} align="center">
                <Button
                  ...
                  disabled={isBidding}
                >
                  {t("bundles.approveBid")}
                  {isBidding && <ButtonCircularProgress />}
                </Button>
              </Box>
            </DialogContentText>
          </DialogContent>
      ...
      {/* mas table */}
      {open5 && (
        <PlaneListPopup open={open5} handleClose={() => setOpen5(false)} />
      )}
      {/* Update Order */}
      {openPlaceBid && (
        <Dialog
          ...
          <DialogActions>
            <Box display="flex" justifyContent="center" width="100%">
              ...
              <Button
                ...
                disabled={isExportingNft}
              >
                {t("bundles.export")} {isExportingNft && <ButtonCircularProgress />}
              </Button>
            </Box>
          </DialogActions>
        </Dialog>
