...
export default function Login({ data, isabout }) {
  ...
  const subscribeUser = async () => {
    if (auth.userLoggedIn && auth.userData?._id && data?._id) {
      try {
        setIsLoading(true);
        const res = await axios.get(Apiconfigs.subscribeProfile + data?._id, {
          headers: {
                ...
                <img
                  ...
                  src={
                    data
                      ? data.profilePic
                        ? data.profilePic
                        : `https://avatars.dicebear.com/api/miniavs/${data?._id ? data?._id : auth?.userData?._id
                        }.svg`
                      : `https://avatars.dicebear.com/api/miniavs/${data?._id ? data?._id : auth?.userData?._id
                      }.svg`
                  }
              ...
              <Box mt={2} mb={1} className={classes.textname}>
                {data?.referralCode && (
                  <Typography variant="h6">
                    {t("profile.referral")} {data?.referralCode} &nbsp;
                    <CopyToClipboard
